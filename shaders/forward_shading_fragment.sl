...
brx_pixel_shader_parameter_end(main)
{
    ...
    brx_float roughness_factor;
    {
        ...
        auxiliary_packed_vectors[0] = brx_byte_address_buffer_load4(g_surface_buffers[3], 0);
        auxiliary_packed_vectors[1] = brx_byte_address_buffer_load4(g_surface_buffers[3], 4 * 4);
        auxiliary_packed_vectors[2].xy = brx_byte_address_buffer_load2(g_surface_buffers[3], 4 * 4 + 4 * 4);
        buffer_texture_flags = auxiliary_packed_vectors[0].x;
        normal_texture_scale = brx_uint_as_float(auxiliary_packed_vectors[0].y);
        emissive_factor = brx_uint_as_float(brx_uint3(auxiliary_packed_vectors[0].zw, auxiliary_packed_vectors[1].x));
    ...
    if(0u != (buffer_texture_flags & Texture_Flag_Enable_Normal_Texture))
    {
        ...
        brx_float3 shading_normal_tangent_space = brx_normalize((brx_sample_2d(g_surface_images[0], g_shared_none_update_set_sampler, texcoord).xyz * 2.0 - brx_float3(1.0, 1.0, 1.0)) * brx_float3(normal_texture_scale, normal_texture_scale, 1.0));
    ...
    {
        emissive *= brx_sample_2d(g_surface_images[1], g_shared_none_update_set_sampler, texcoord).xyz;
    }
    ...
    {
        base_color *= brx_sample_2d(g_surface_images[2], g_shared_none_update_set_sampler, texcoord).xyz;
    }
    ...
    {
        brx_float2 metallic_roughness = brx_sample_2d(g_surface_images[3], g_shared_none_update_set_sampler, texcoord).bg;
