...
{
    brx_uint buffer_texture_flags = brx_byte_address_buffer_load(g_surface_buffers[Forward_Shading_Surface_Buffer_Auxiliary_Index], 0);

    brx_uint vertex_index;
    brx_branch
    if(0u != (buffer_texture_flags & Surface_Buffer_Flag_UInt16_Index))
    {
        brx_uint index_buffer_offset = (0u == ((g_surface_uint16_index_buffer_stride * brx_uint(brx_vertex_index)) & 3u)) ? (g_surface_uint16_index_buffer_stride * brx_uint(brx_vertex_index)) : ((g_surface_uint16_index_buffer_stride * brx_uint(brx_vertex_index)) - 2u);
        brx_uint packed_vector_index_buffer = brx_byte_address_buffer_load(g_surface_buffers[Forward_Shading_Surface_Buffer_Index_Index], index_buffer_offset);
        brx_uint2 unpacked_vector_index_buffer = brx_R16G16_UINT_to_UINT2(packed_vector_index_buffer);
        vertex_index = (0u == ((g_surface_uint16_index_buffer_stride * brx_uint(brx_vertex_index)) & 3u)) ? unpacked_vector_index_buffer.x : unpacked_vector_index_buffer.y;
    }
    else
    {
        brx_uint index_buffer_offset = g_surface_uint32_index_buffer_stride * brx_uint(brx_vertex_index);
        vertex_index = brx_byte_address_buffer_load(g_surface_buffers[Forward_Shading_Surface_Buffer_Index_Index], index_buffer_offset);
    }
    ...
    brx_float3 vertex_position_model_space;
    {
        ...
        brx_uint3 packed_vector_vertex_position_binding = brx_byte_address_buffer_load3(g_surface_buffers[Forward_Shading_Surface_Buffer_Vertex_Position_Index], vertex_position_buffer_offset);
    ...
    brx_float2 vertex_texcoord;
    {
        ...
        brx_uint3 packed_vector_vertex_varying_binding = brx_byte_address_buffer_load3(g_surface_buffers[Forward_Shading_Surface_Buffer_Vertex_Varying_Index], vertex_varying_buffer_offset);
