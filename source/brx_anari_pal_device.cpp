...
#include "../shaders/spirv/post_processing_fragment.inl"
    ...
    assert(NULL == this->m_place_holder_asset_buffer);
    {
        mcrt_vector<uint8_t> data(static_cast<size_t>(1U));
        this->m_place_holder_asset_buffer = this->internal_create_asset_buffer(data.data(), 1U);
    }

    assert(NULL == this->m_place_holder_asset_image);
    {
        mcrt_vector<uint32_t> pixel_data(static_cast<size_t>(1U * 1U));
...
void brx_anari_pal_device::uninit()
{
    ...
    for (uint32_t frame_throttling_index = 0U; frame_throttling_index < INTERNAL_FRAME_THROTTLING_COUNT; ++frame_throttling_index)
    {
        ...
        for (brx_pal_descriptor_set *(&pending_destroy_descriptor_set) : this->m_pending_destroy_descriptor_sets[frame_throttling_index])
        {
        ...
        for (brx_pal_uniform_upload_buffer *(&pending_destroy_uniform_upload_buffer) : this->m_pending_destroy_uniform_upload_buffers[frame_throttling_index])
        {
        ...
        for (brx_pal_storage_intermediate_buffer *(&pending_destroy_storage_intermediate_buffer) : this->m_pending_destroy_storage_intermediate_buffers[frame_throttling_index])
        {
        ...
        for (brx_pal_storage_asset_buffer *(&pending_destroy_storage_asset_buffer) : this->m_pending_destroy_storage_asset_buffers[frame_throttling_index])
        {
        ...
        for (brx_pal_sampled_asset_image *(&pending_destroy_sampled_asset_image) : this->m_pending_destroy_sampled_asset_images[frame_throttling_index])
        {
