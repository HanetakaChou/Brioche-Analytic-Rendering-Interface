...
brx_anari_pal_device::brx_anari_pal_device()
    : m_device(NULL),
      ...
      m_environment_lighting_sh_projection_descriptor_set_layout_per_environment_lighting_update(NULL),
      m_environment_lighting_sh_projection_pipeline_layout(NULL),
      m_environment_lighting_skybox_none_update_set_uniform_buffer(NULL),
      m_environment_lighting_skybox_descriptor_set_none_update(NULL),
      m_environment_lighting_skybox_descriptor_set_layout_per_environment_lighting_update(NULL),
      m_environment_lighting_skybox_pipeline_layout(NULL),
      m_forward_shading_none_update_set_uniform_buffer(NULL),
      m_forward_shading_descriptor_set_none_update(NULL),
      m_forward_shading_descriptor_set_layout_per_environment_lighting_update(NULL),
      ...
      m_environment_lighting_skybox_pipeline(NULL),
      ...
      m_hdri_light_environment_lighting_sh_projection_descriptor_set_per_environment_lighting_update(NULL),
      m_hdri_light_environment_lighting_skybox_per_environment_lighting_update_set_uniform_buffer(NULL),
      m_hdri_light_environment_lighting_skybox_descriptor_set_per_environment_lighting_update(NULL),
      m_hdri_light_forward_shading_per_environment_lighting_update_set_uniform_buffer(NULL),
      m_hdri_light_forward_shading_descriptor_set_per_environment_lighting_update(NULL)
{
}
...
brx_anari_pal_device::~brx_anari_pal_device()
{
    ...
    assert(NULL == this->m_environment_lighting_sh_projection_descriptor_set_layout_per_environment_lighting_update);
    assert(NULL == this->m_environment_lighting_sh_projection_pipeline_layout);
    assert(NULL == this->m_environment_lighting_skybox_none_update_set_uniform_buffer);
    assert(NULL == this->m_environment_lighting_skybox_descriptor_set_none_update);
    assert(NULL == this->m_environment_lighting_skybox_descriptor_set_layout_per_environment_lighting_update);
    assert(NULL == this->m_environment_lighting_skybox_pipeline_layout);
    ...
    assert(NULL == this->m_forward_shading_descriptor_set_layout_per_environment_lighting_update);
    ...
    assert(NULL == this->m_hdri_light_environment_lighting_skybox_per_environment_lighting_update_set_uniform_buffer);
    assert(NULL == this->m_hdri_light_environment_lighting_skybox_descriptor_set_per_environment_lighting_update);
    assert(NULL == this->m_hdri_light_forward_shading_per_environment_lighting_update_set_uniform_buffer);
    assert(NULL == this->m_hdri_light_forward_shading_descriptor_set_per_environment_lighting_update);
}
...
void brx_anari_pal_device::init(void *wsi_connection)
{
    ...
    // Scene Renderer
    {
        ...
        {
            brx_pal_descriptor_set_layout *environment_lighting_skybox_descriptor_set_layout_none_update = NULL;
            ...
            // Environment Lighting SH Projection Descriptor/Pipeline Layout
            {
                assert(NULL == this->m_environment_lighting_sh_projection_descriptor_set_layout_per_environment_lighting_update);
                BRX_PAL_DESCRIPTOR_SET_LAYOUT_BINDING const environment_lighting_sh_projection_descriptor_set_layout_per_environment_lighting_update_bindings[] = {
                    {0U, BRX_PAL_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 1U},
                    {1U, BRX_PAL_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1U}};
                this->m_environment_lighting_sh_projection_descriptor_set_layout_per_environment_lighting_update = this->m_device->create_descriptor_set_layout(sizeof(environment_lighting_sh_projection_descriptor_set_layout_per_environment_lighting_update_bindings) / sizeof(environment_lighting_sh_projection_descriptor_set_layout_per_environment_lighting_update_bindings[0]), environment_lighting_sh_projection_descriptor_set_layout_per_environment_lighting_update_bindings);

                assert(NULL == this->m_environment_lighting_sh_projection_pipeline_layout);
                brx_pal_descriptor_set_layout *const environment_lighting_sh_projection_pipeline_descriptor_set_layouts[] = {
                    this->m_environment_lighting_sh_projection_descriptor_set_layout_per_environment_lighting_update};
                this->m_environment_lighting_sh_projection_pipeline_layout = this->m_device->create_pipeline_layout(sizeof(environment_lighting_sh_projection_pipeline_descriptor_set_layouts) / sizeof(environment_lighting_sh_projection_pipeline_descriptor_set_layouts[0]), environment_lighting_sh_projection_pipeline_descriptor_set_layouts);
            }

            // Environment Lighting Skybox Descriptor/Pipeline Layout
            {
                assert(NULL == environment_lighting_skybox_descriptor_set_layout_none_update);
                BRX_PAL_DESCRIPTOR_SET_LAYOUT_BINDING const environment_lighting_skybox_descriptor_set_layout_none_update_bindings[] = {
                    {0U, BRX_PAL_DESCRIPTOR_TYPE_DYNAMIC_UNIFORM_BUFFER, 1U},
                    {1U, BRX_PAL_DESCRIPTOR_TYPE_SAMPLER, 1U}};
                environment_lighting_skybox_descriptor_set_layout_none_update = this->m_device->create_descriptor_set_layout(sizeof(environment_lighting_skybox_descriptor_set_layout_none_update_bindings) / sizeof(environment_lighting_skybox_descriptor_set_layout_none_update_bindings[0]), environment_lighting_skybox_descriptor_set_layout_none_update_bindings);

                assert(NULL == this->m_environment_lighting_skybox_descriptor_set_layout_per_environment_lighting_update);
                BRX_PAL_DESCRIPTOR_SET_LAYOUT_BINDING const environment_lighting_skybox_descriptor_set_layout_per_environment_lighting_update_bindings[] = {
                    {0U, BRX_PAL_DESCRIPTOR_TYPE_DYNAMIC_UNIFORM_BUFFER, 1U},
                    {1U, BRX_PAL_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 1U}};
                this->m_environment_lighting_skybox_descriptor_set_layout_per_environment_lighting_update = this->m_device->create_descriptor_set_layout(sizeof(environment_lighting_skybox_descriptor_set_layout_per_environment_lighting_update_bindings) / sizeof(environment_lighting_skybox_descriptor_set_layout_per_environment_lighting_update_bindings[0]), environment_lighting_skybox_descriptor_set_layout_per_environment_lighting_update_bindings);

                assert(NULL == this->m_environment_lighting_skybox_pipeline_layout);
                brx_pal_descriptor_set_layout *const environment_lighting_skybox_pipeline_descriptor_set_layouts[] = {
                    environment_lighting_skybox_descriptor_set_layout_none_update,
                    this->m_environment_lighting_skybox_descriptor_set_layout_per_environment_lighting_update};
                this->m_environment_lighting_skybox_pipeline_layout = this->m_device->create_pipeline_layout(sizeof(environment_lighting_skybox_pipeline_descriptor_set_layouts) / sizeof(environment_lighting_skybox_pipeline_descriptor_set_layouts[0]), environment_lighting_skybox_pipeline_descriptor_set_layouts);
            }

            // Environment Lighting Skybox None Update Uniform Buffer
            {
                assert(NULL == this->m_environment_lighting_skybox_none_update_set_uniform_buffer);

                this->m_environment_lighting_skybox_none_update_set_uniform_buffer = this->m_device->create_uniform_upload_buffer(internal_align_up(static_cast<uint32_t>(sizeof(environment_lighting_skybox_none_update_set_uniform_buffer_binding)), this->m_uniform_upload_buffer_offset_alignment) * INTERNAL_FRAME_THROTTLING_COUNT);
            }

            // Environment Lighting Skybox None Update Descriptor
            {
                assert(NULL == this->m_environment_lighting_skybox_descriptor_set_none_update);
                this->m_environment_lighting_skybox_descriptor_set_none_update = this->m_device->create_descriptor_set(environment_lighting_skybox_descriptor_set_layout_none_update, 0U);
            }

            // Write Environment Lighting Skybox None Update Descriptor
            {
                {
                    constexpr uint32_t const dynamic_uniform_buffers_range = sizeof(environment_lighting_skybox_none_update_set_uniform_buffer_binding);

                    this->m_device->write_descriptor_set(this->m_environment_lighting_skybox_descriptor_set_none_update, 0U, BRX_PAL_DESCRIPTOR_TYPE_DYNAMIC_UNIFORM_BUFFER, 0U, 1U, &this->m_environment_lighting_skybox_none_update_set_uniform_buffer, &dynamic_uniform_buffers_range, NULL, NULL, NULL, NULL, NULL, NULL);
                }

                {
                    this->m_device->write_descriptor_set(this->m_environment_lighting_skybox_descriptor_set_none_update, 1U, BRX_PAL_DESCRIPTOR_TYPE_SAMPLER, 0U, 1U, NULL, NULL, NULL, NULL, NULL, NULL, &this->m_shared_none_update_set_linear_clamp_sampler, NULL);
                }
            }
            ...
            // Forward Shading Descriptor/Pipeline Layout
            {
                ...
                assert(NULL == this->m_forward_shading_descriptor_set_layout_per_environment_lighting_update);
                BRX_PAL_DESCRIPTOR_SET_LAYOUT_BINDING const forward_shading_descriptor_set_layout_per_environment_lighting_update_bindings[] = {
                    {0U, BRX_PAL_DESCRIPTOR_TYPE_DYNAMIC_UNIFORM_BUFFER, 1U},
                    {1U, BRX_PAL_DESCRIPTOR_TYPE_READ_ONLY_STORAGE_BUFFER, 1U}};
                this->m_forward_shading_descriptor_set_layout_per_environment_lighting_update = this->m_device->create_descriptor_set_layout(sizeof(forward_shading_descriptor_set_layout_per_environment_lighting_update_bindings) / sizeof(forward_shading_descriptor_set_layout_per_environment_lighting_update_bindings[0]), forward_shading_descriptor_set_layout_per_environment_lighting_update_bindings);
                ...
                brx_pal_descriptor_set_layout *const forward_shading_pipeline_descriptor_set_layouts[] = {
                    ...
                    this->m_forward_shading_descriptor_set_layout_per_environment_lighting_update,
                    this->m_forward_shading_descriptor_set_layout_per_surface_group_update,
            ...
            {
                {
                    ...
                    this->m_device->write_descriptor_set(this->m_forward_shading_descriptor_set_none_update, 0U, BRX_PAL_DESCRIPTOR_TYPE_DYNAMIC_UNIFORM_BUFFER, 0U, 1U, &this->m_forward_shading_none_update_set_uniform_buffer, &dynamic_uniform_buffers_range, NULL, NULL, NULL, NULL, NULL, NULL);
                }

                {
                    this->m_device->write_descriptor_set(this->m_forward_shading_descriptor_set_none_update, 1U, BRX_PAL_DESCRIPTOR_TYPE_SAMPLER, 0U, 1U, NULL, NULL, NULL, NULL, NULL, NULL, &this->m_shared_none_update_set_linear_clamp_sampler, NULL);
                }
            }
            ...
            // Write Post Processing None Update Descriptor
            {
                ...
                this->m_device->write_descriptor_set(this->m_post_processing_descriptor_set_none_update, 0U, BRX_PAL_DESCRIPTOR_TYPE_DYNAMIC_UNIFORM_BUFFER, 0U, 1U, &this->m_post_processing_none_update_set_uniform_buffer, &dynamic_uniform_buffers_range, NULL, NULL, NULL, NULL, NULL, NULL);
            }

            this->m_device->destroy_descriptor_set_layout(environment_lighting_skybox_descriptor_set_layout_none_update);
            environment_lighting_skybox_descriptor_set_layout_none_update = NULL;
...
#include "../shaders/spirv/deforming_compute.inl"
            ...
            // Environment Lighting SH Projection Pipeline
            {
                assert(NULL == this->m_environment_lighting_sh_projection_clear_pipeline);
                switch (this->m_device->get_backend_name())
                {
                case BRX_PAL_BACKEND_NAME_D3D12:
                {
#include "../shaders/dxil/environment_lighting_sh_projection_clear_compute.inl"
                    this->m_environment_lighting_sh_projection_clear_pipeline = this->m_device->create_compute_pipeline(this->m_environment_lighting_sh_projection_pipeline_layout, sizeof(environment_lighting_sh_projection_clear_compute_shader_module_code), environment_lighting_sh_projection_clear_compute_shader_module_code);
                }
                break;
                case BRX_PAL_BACKEND_NAME_VK:
                {
#include "../shaders/spirv/environment_lighting_sh_projection_clear_compute.inl"
                    this->m_environment_lighting_sh_projection_clear_pipeline = this->m_device->create_compute_pipeline(this->m_environment_lighting_sh_projection_pipeline_layout, sizeof(environment_lighting_sh_projection_clear_compute_shader_module_code), environment_lighting_sh_projection_clear_compute_shader_module_code);
                }
                break;
                default:
                ...
                case BRX_PAL_BACKEND_NAME_D3D12:
                {
#include "../shaders/dxil/environment_lighting_sh_projection_equirectangular_map_compute.inl"
                    this->m_environment_lighting_sh_projection_equirectangular_map_pipeline = this->m_device->create_compute_pipeline(this->m_environment_lighting_sh_projection_pipeline_layout, sizeof(environment_lighting_sh_projection_equirectangular_map_compute_shader_module_code), environment_lighting_sh_projection_equirectangular_map_compute_shader_module_code);
                }
                break;
                case BRX_PAL_BACKEND_NAME_VK:
                {
#include "../shaders/spirv/environment_lighting_sh_projection_equirectangular_map_compute.inl"
                    this->m_environment_lighting_sh_projection_equirectangular_map_pipeline = this->m_device->create_compute_pipeline(this->m_environment_lighting_sh_projection_pipeline_layout, sizeof(environment_lighting_sh_projection_equirectangular_map_compute_shader_module_code), environment_lighting_sh_projection_equirectangular_map_compute_shader_module_code);
                }
                break;
                default:
                ...
                case BRX_PAL_BACKEND_NAME_D3D12:
                {
#include "../shaders/dxil/environment_lighting_sh_projection_octahedral_map_compute.inl"
                    this->m_environment_lighting_sh_projection_octahedral_map_pipeline = this->m_device->create_compute_pipeline(this->m_environment_lighting_sh_projection_pipeline_layout, sizeof(environment_lighting_sh_projection_octahedral_map_compute_shader_module_code), environment_lighting_sh_projection_octahedral_map_compute_shader_module_code);
                }
                break;
                case BRX_PAL_BACKEND_NAME_VK:
                {
#include "../shaders/spirv/environment_lighting_sh_projection_octahedral_map_compute.inl"
                    this->m_environment_lighting_sh_projection_octahedral_map_pipeline = this->m_device->create_compute_pipeline(this->m_environment_lighting_sh_projection_pipeline_layout, sizeof(environment_lighting_sh_projection_octahedral_map_compute_shader_module_code), environment_lighting_sh_projection_octahedral_map_compute_shader_module_code);
                }
                break;
                default:
            ...
            // Environment Lighting Skybox Pipeline
            {
                assert(NULL == this->m_environment_lighting_skybox_pipeline);
...
#include "../shaders/dxil/environment_lighting_skybox_fragment.inl"
                    this->m_environment_lighting_skybox_pipeline = this->m_device->create_graphics_pipeline(this->m_forward_shading_render_pass, this->m_environment_lighting_skybox_pipeline_layout, sizeof(environment_lighting_skybox_vertex_shader_module_code), environment_lighting_skybox_vertex_shader_module_code, sizeof(environment_lighting_skybox_fragment_shader_module_code), environment_lighting_skybox_fragment_shader_module_code, false, true, BRX_PAL_GRAPHICS_PIPELINE_DEPTH_COMPARE_OPERATION_DISABLE, BRX_PAL_GRAPHICS_PIPELINE_BLEND_OPERATION_DISABLE);
                }
                break;
                case BRX_PAL_BACKEND_NAME_VK:
                {
#include "../shaders/spirv/environment_lighting_skybox_vertex.inl"
#include "../shaders/spirv/environment_lighting_skybox_fragment.inl"
                    this->m_environment_lighting_skybox_pipeline = this->m_device->create_graphics_pipeline(this->m_forward_shading_render_pass, this->m_environment_lighting_skybox_pipeline_layout, sizeof(environment_lighting_skybox_vertex_shader_module_code), environment_lighting_skybox_vertex_shader_module_code, sizeof(environment_lighting_skybox_fragment_shader_module_code), environment_lighting_skybox_fragment_shader_module_code, false, true, BRX_PAL_GRAPHICS_PIPELINE_DEPTH_COMPARE_OPERATION_DISABLE, BRX_PAL_GRAPHICS_PIPELINE_BLEND_OPERATION_DISABLE);
                }
                break;
                default:
            ...
            {
                // TODO: enable backface culling one day???
...
#include "../shaders/dxil/post_processing_fragment.inl"
                    this->m_post_processing_pipeline = this->m_device->create_graphics_pipeline(this->m_post_processing_render_pass, this->m_post_processing_pipeline_layout, sizeof(post_processing_vertex_shader_module_code), post_processing_vertex_shader_module_code, sizeof(post_processing_fragment_shader_module_code), post_processing_fragment_shader_module_code, false, true, BRX_PAL_GRAPHICS_PIPELINE_DEPTH_COMPARE_OPERATION_DISABLE, BRX_PAL_GRAPHICS_PIPELINE_BLEND_OPERATION_DISABLE);
                }
                break;
                case BRX_PAL_BACKEND_NAME_VK:
                {
...
#include "../shaders/spirv/post_processing_fragment.inl"
    ...
    assert(NULL == this->m_hdri_light_environment_lighting_skybox_per_environment_lighting_update_set_uniform_buffer);
    this->m_hdri_light_environment_lighting_skybox_per_environment_lighting_update_set_uniform_buffer = this->m_device->create_uniform_upload_buffer(internal_align_up(static_cast<uint32_t>(sizeof(environment_lighting_skybox_per_environment_lighting_update_set_uniform_buffer_binding)), this->m_uniform_upload_buffer_offset_alignment) * INTERNAL_FRAME_THROTTLING_COUNT);
    ...
    assert(NULL == this->m_hdri_light_forward_shading_per_environment_lighting_update_set_uniform_buffer);
    this->m_hdri_light_forward_shading_per_environment_lighting_update_set_uniform_buffer = this->m_device->create_uniform_upload_buffer(internal_align_up(static_cast<uint32_t>(sizeof(forward_shading_per_environment_lighting_update_set_uniform_buffer_binding)), this->m_uniform_upload_buffer_offset_alignment) * INTERNAL_FRAME_THROTTLING_COUNT);

    assert(NULL == this->m_hdri_light_forward_shading_descriptor_set_per_environment_lighting_update);
    assert(NULL != this->m_hdri_light_irradiance_coefficients);
    this->m_hdri_light_forward_shading_descriptor_set_per_environment_lighting_update = this->create_forward_shading_per_environment_lighting_descriptor_set(this->m_hdri_light_forward_shading_per_environment_lighting_update_set_uniform_buffer, this->m_hdri_light_irradiance_coefficients->get_read_only_storage_buffer());
}

void brx_anari_pal_device::uninit()
{
    assert(NULL != this->m_hdri_light_forward_shading_descriptor_set_per_environment_lighting_update);
    this->destroy_descriptor_set(this->m_hdri_light_forward_shading_descriptor_set_per_environment_lighting_update);
    this->m_hdri_light_forward_shading_descriptor_set_per_environment_lighting_update = NULL;

    assert(NULL != this->m_hdri_light_forward_shading_per_environment_lighting_update_set_uniform_buffer);
    this->m_device->destroy_uniform_upload_buffer(this->m_hdri_light_forward_shading_per_environment_lighting_update_set_uniform_buffer);
    this->m_hdri_light_forward_shading_per_environment_lighting_update_set_uniform_buffer = NULL;
    ...
    assert(NULL != this->m_hdri_light_environment_lighting_skybox_per_environment_lighting_update_set_uniform_buffer);
    this->m_device->destroy_uniform_upload_buffer(this->m_hdri_light_environment_lighting_skybox_per_environment_lighting_update_set_uniform_buffer);
    this->m_hdri_light_environment_lighting_skybox_per_environment_lighting_update_set_uniform_buffer = NULL;
    ...
    // Scene Renderer
    {
        // Render Pass and Pipeline
        {
            ...
            assert(NULL != this->m_environment_lighting_skybox_pipeline);
            this->m_device->destroy_graphics_pipeline(this->m_environment_lighting_skybox_pipeline);
            this->m_environment_lighting_skybox_pipeline = NULL;
        ...
        // Descriptor/Pipeline Layout and None Update Descriptor
        {
            ...
            {
                assert(NULL != this->m_environment_lighting_sh_projection_pipeline_layout);
                this->m_device->destroy_pipeline_layout(this->m_environment_lighting_sh_projection_pipeline_layout);
                this->m_environment_lighting_sh_projection_pipeline_layout = NULL;

                assert(NULL != this->m_environment_lighting_sh_projection_descriptor_set_layout_per_environment_lighting_update);
                this->m_device->destroy_descriptor_set_layout(this->m_environment_lighting_sh_projection_descriptor_set_layout_per_environment_lighting_update);
                this->m_environment_lighting_sh_projection_descriptor_set_layout_per_environment_lighting_update = NULL;
            }

            // Environment Lighting Skybox None Update Descriptor
            {
                assert(NULL != this->m_environment_lighting_skybox_descriptor_set_none_update);
                this->m_device->destroy_descriptor_set(this->m_environment_lighting_skybox_descriptor_set_none_update);
                this->m_environment_lighting_skybox_descriptor_set_none_update = NULL;
            }

            // Environment Lighting Skybox None Update Uniform Buffer
            {
                assert(NULL != this->m_environment_lighting_skybox_none_update_set_uniform_buffer);
                this->m_device->destroy_uniform_upload_buffer(this->m_environment_lighting_skybox_none_update_set_uniform_buffer);
                this->m_environment_lighting_skybox_none_update_set_uniform_buffer = NULL;
            }

            // Environment Lighting Skybox Projection Descriptor/Pipeline Layout
            {
                assert(NULL != this->m_environment_lighting_skybox_pipeline_layout);
                this->m_device->destroy_pipeline_layout(this->m_environment_lighting_skybox_pipeline_layout);
                this->m_environment_lighting_skybox_pipeline_layout = NULL;

                assert(NULL != this->m_environment_lighting_skybox_descriptor_set_layout_per_environment_lighting_update);
                this->m_device->destroy_descriptor_set_layout(this->m_environment_lighting_skybox_descriptor_set_layout_per_environment_lighting_update);
                this->m_environment_lighting_skybox_descriptor_set_layout_per_environment_lighting_update = NULL;
            }
            ...
            // Forward Shading Descriptor/Pipeline Layout
            {
                ...
                assert(NULL != this->m_forward_shading_descriptor_set_layout_per_environment_lighting_update);
                this->m_device->destroy_descriptor_set_layout(this->m_forward_shading_descriptor_set_layout_per_environment_lighting_update);
                this->m_forward_shading_descriptor_set_layout_per_environment_lighting_update = NULL;
...
void brx_anari_pal_device::renderer_render_frame(bool ui_view)
{
    ...
    // Scene Renderer
    {
        ...
        this->render_hdri_light_sh_projection(command_buffer);
        ...
        // Forward Shading Pass
        {
            DirectX::XMFLOAT4X4 environment_map_to_world_transform;
            {
                ...
                environment_map_to_world_transform.m[1][0] = this->m_hdri_light_direction.m_y;
                environment_map_to_world_transform.m[2][0] = this->m_hdri_light_direction.m_z;
                environment_map_to_world_transform.m[3][0] = 0.0F;
                environment_map_to_world_transform.m[0][1] = environment_map_left.x;
                environment_map_to_world_transform.m[1][1] = environment_map_left.y;
                environment_map_to_world_transform.m[2][1] = environment_map_left.z;
                environment_map_to_world_transform.m[3][1] = 0.0F;
                environment_map_to_world_transform.m[0][2] = this->m_hdri_light_up.m_x;
                environment_map_to_world_transform.m[1][2] = this->m_hdri_light_up.m_y;
            ...
            {
                command_buffer->bind_graphics_pipeline(this->m_environment_lighting_skybox_pipeline);

                // Update Uniform Buffer
                {
                    environment_lighting_skybox_none_update_set_uniform_buffer_binding *const environment_lighting_skybox_none_update_set_uniform_buffer_destination = reinterpret_cast<environment_lighting_skybox_none_update_set_uniform_buffer_binding *>(reinterpret_cast<uintptr_t>(this->m_environment_lighting_skybox_none_update_set_uniform_buffer->get_host_memory_range_base()) + internal_align_up(static_cast<uint32_t>(sizeof(environment_lighting_skybox_none_update_set_uniform_buffer_binding)), this->m_uniform_upload_buffer_offset_alignment) * this->m_frame_throttling_index);
                    environment_lighting_skybox_none_update_set_uniform_buffer_destination->g_inverse_view_transform = camera_inverse_view_transform;
                    environment_lighting_skybox_none_update_set_uniform_buffer_destination->g_inverse_projection_transform = camera_inverse_projection_transform;
                }

                // Update Uniform Buffer
                {
                    environment_lighting_skybox_per_environment_lighting_update_set_uniform_buffer_binding *const environment_lighting_skybox_per_environment_lighting_update_set_uniform_buffer_destination = reinterpret_cast<environment_lighting_skybox_per_environment_lighting_update_set_uniform_buffer_binding *>(reinterpret_cast<uintptr_t>(this->m_hdri_light_environment_lighting_skybox_per_environment_lighting_update_set_uniform_buffer->get_host_memory_range_base()) + internal_align_up(static_cast<uint32_t>(sizeof(environment_lighting_skybox_per_environment_lighting_update_set_uniform_buffer_binding)), this->m_uniform_upload_buffer_offset_alignment) * this->m_frame_throttling_index);

                    environment_lighting_skybox_per_environment_lighting_update_set_uniform_buffer_destination->g_world_to_environment_map_transform = environment_map_to_world_transform;

                    environment_lighting_skybox_per_environment_lighting_update_set_uniform_buffer_destination->g_environment_map_layout = static_cast<uint32_t>(this->m_hdri_light_layout);
                }

                {
                    brx_pal_descriptor_set const *const descriptor_sets[] = {
                        this->m_environment_lighting_skybox_descriptor_set_none_update,
                        this->m_hdri_light_environment_lighting_skybox_descriptor_set_per_environment_lighting_update};

                    uint32_t const dynamic_offsets[] = {
                        internal_align_up(static_cast<uint32_t>(sizeof(environment_lighting_skybox_none_update_set_uniform_buffer_binding)), this->m_uniform_upload_buffer_offset_alignment) * this->m_frame_throttling_index,
                        internal_align_up(static_cast<uint32_t>(sizeof(environment_lighting_skybox_per_environment_lighting_update_set_uniform_buffer_binding)), this->m_uniform_upload_buffer_offset_alignment) * this->m_frame_throttling_index};

                    command_buffer->bind_graphics_descriptor_sets(this->m_environment_lighting_skybox_pipeline_layout, sizeof(descriptor_sets) / sizeof(descriptor_sets[0]), descriptor_sets, sizeof(dynamic_offsets) / sizeof(dynamic_offsets[0]), dynamic_offsets);
                }
            ...
            // Forward Shading
            {
                ...
                // Update Uniform Buffer
                {
                    forward_shading_per_environment_lighting_update_set_uniform_buffer_binding *const forward_shading_per_environment_lighting_update_set_uniform_buffer_destination = reinterpret_cast<forward_shading_per_environment_lighting_update_set_uniform_buffer_binding *>(reinterpret_cast<uintptr_t>(this->m_hdri_light_forward_shading_per_environment_lighting_update_set_uniform_buffer->get_host_memory_range_base()) + internal_align_up(static_cast<uint32_t>(sizeof(forward_shading_per_environment_lighting_update_set_uniform_buffer_binding)), this->m_uniform_upload_buffer_offset_alignment) * this->m_frame_throttling_index);

                    forward_shading_per_environment_lighting_update_set_uniform_buffer_destination->g_world_to_environment_map_transform = environment_map_to_world_transform;
                }
                        ...
                        for (uint32_t surface_index = 0U; surface_index < surface_count; ++surface_index)
                        {
...

                            {
                                ...
                                uint32_t const dynamic_offsets[] = {
                                    ...
                                    internal_align_up(static_cast<uint32_t>(sizeof(forward_shading_per_environment_lighting_update_set_uniform_buffer_binding)), this->m_uniform_upload_buffer_offset_alignment) * this->m_frame_throttling_index,
        ...
        // Post Processing Pass
        {
            // Update Uniform Buffer
            {
                ...
                post_processing_none_update_set_uniform_buffer_destination->g_view_port_width = static_cast<float>(this->m_intermediate_width);
                post_processing_none_update_set_uniform_buffer_destination->g_view_port_height = static_cast<float>(this->m_intermediate_height);
            }
            ...
            brx_pal_descriptor_set *const descriptor_sets[] = {
                this->m_post_processing_descriptor_set_none_update};

            uint32_t const dynamic_offsets[] = {
                internal_align_up(static_cast<uint32_t>(sizeof(post_processing_none_update_set_uniform_buffer_binding)), this->m_uniform_upload_buffer_offset_alignment) * this->m_frame_throttling_index};
