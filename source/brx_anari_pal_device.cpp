...
void brx_anari_pal_device::init(void *wsi_connection)
{
    ...
    // Scene Renderer
    {
        // Uniform Buffer
        {
            ...
            uint32_t const uniform_upload_buffer_offset_alignment = this->m_device->get_uniform_upload_buffer_offset_alignment();

            this->m_shared_none_update_set_uniform_buffer = this->m_device->create_uniform_upload_buffer(internal_align_up(static_cast<uint32_t>(sizeof(shared_none_update_set_uniform_buffer_binding)), uniform_upload_buffer_offset_alignment) * INTERNAL_FRAME_THROTTLING_COUNT);
        }
...
brx_anari_buffer *brx_anari_pal_device::new_buffer(BRX_ANARI_BUFFER_FORMAT format, void *data_base, uint32_t data_size)
{
    return NULL;
}

void brx_anari_pal_device::release_buffer(brx_anari_buffer *buffer)
{
}

brx_anari_image *brx_anari_pal_device::new_image(BRX_ANARI_IMAGE_FORMAT format, void *pixel_data, uint32_t width, uint32_t height)
{
    ...
    new_unwrapped_image->init(this->m_device, format, pixel_data, width, height);
    return new_unwrapped_image;
}

void brx_anari_pal_device::release_image(brx_anari_image *wrapped_image)
{
    brx_pal_sampled_asset_image *pending_destroy_sampled_asset_image = NULL;
    {
        assert(NULL != wrapped_image);
        brx_anari_pal_image *delete_unwrapped_image = static_cast<brx_anari_pal_image *>(wrapped_image);

        delete_unwrapped_image->uninit(&pending_destroy_sampled_asset_image);
    ...
    this->m_pending_destroy_sampled_asset_images[previous_frame_throttling_index].push_back(pending_destroy_sampled_asset_image);
}
...
void brx_anari_pal_device::renderer_render_frame()
{
    ...
    // Scene Renderer
    {
        ...
        uint32_t const uniform_upload_buffer_offset_alignment = this->m_device->get_uniform_upload_buffer_offset_alignment();
        uint32_t const frame_throttling_index = this->m_frame_throttling_index;
        ...
        // Post Processing Pass
        {
            ...
            brx_pal_descriptor_set *const descritor_sets[] = {this->m_post_processing_descriptor_set_none_update};
            uint32_t const dynamic_offsets[] = {internal_align_up(static_cast<uint32_t>(sizeof(shared_none_update_set_uniform_buffer_binding)), uniform_upload_buffer_offset_alignment) * frame_throttling_index};
            command_buffer->bind_graphics_descriptor_sets(this->m_post_processing_pipeline_layout, sizeof(descritor_sets) / sizeof(descritor_sets[0]), descritor_sets, sizeof(dynamic_offsets) / sizeof(dynamic_offsets[0]), dynamic_offsets);
