...
public:
	...
	inline brx_pal_storage_asset_buffer *create_asset_buffer(void const *const data_base, uint32_t const data_size);
	inline void destroy_asset_buffer(brx_pal_storage_asset_buffer *const buffer);

	inline brx_pal_sampled_asset_image *internal_create_asset_image(BRX_ANARI_IMAGE_FORMAT const format, void const *const pixel_data, uint32_t const width, uint32_t const height);
	inline void internal_destroy_asset_image(brx_pal_sampled_asset_image *const image);
...
public:
	...
	inline ~brx_anari_pal_image();
	inline void init(brx_anari_pal_device *device, BRX_ANARI_IMAGE_FORMAT format, void *pixel_data, uint32_t width, uint32_t height);
