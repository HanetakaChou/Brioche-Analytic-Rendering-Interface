...
brx_anari_pal_image::brx_anari_pal_image() : m_image(NULL)
{
}

brx_anari_pal_image::~brx_anari_pal_image()
{
    assert(NULL == this->m_image);
}

void brx_anari_pal_image::init(brx_pal_device *device, BRX_ANARI_IMAGE_FORMAT origin_format, void const *const origin_pixel_data, uint32_t const origin_width, uint32_t const origin_height)
{
    assert(BRX_ANARI_IMAGE_FORMAT_R8G8B8A8_UNORM == origin_format || BRX_ANARI_IMAGE_FORMAT_R8G8B8A8_SRGB == origin_format);
    ...
    {
        {
            ...
            image_staging_upload_buffer = device->create_staging_upload_buffer(total_bytes);
        }
        ...
        // zeroth
        {
            ...
            if ((origin_width == width) && (origin_height == height))
            {
                ...
                size_t const total_size = sizeof(uint32_t) * pixel_data.size();
        ...
        for (uint32_t mip_level_index = 1U; mip_level_index < mip_level_count; ++mip_level_index)
        {
            ...
            {
                int const dims = 2;
                int const source_sizes[dims] = {static_cast<int>(previous_width), static_cast<int>(previous_height)};
                int const type = CV_MAKETYPE(CV_8U, 4);
    ...
    brx_pal_sampled_asset_image *const destination_sampled_asset_image = device->create_sampled_asset_image(format, zeroth_width, zeroth_height, mip_level_count);

    {
        ...
        mcrt_vector<BRX_PAL_SAMPLED_ASSET_IMAGE_SUBRESOURCE> uploaded_sampled_asset_image_subresources(static_cast<size_t>(mip_level_count));

        for (uint32_t mip_level_index = 0U; mip_level_index < mip_level_count; ++mip_level_index)
        {
            upload_command_buffer->upload_from_staging_upload_buffer_to_sampled_asset_image(destination_sampled_asset_image, format, zeroth_width, zeroth_height, mip_level_index, image_staging_upload_buffer, subresource_memcpy_dests[mip_level_index].staging_upload_buffer_offset, subresource_memcpy_dests[mip_level_index].output_row_pitch, subresource_memcpy_dests[mip_level_index].output_row_count);

            uploaded_sampled_asset_image_subresources[mip_level_index] = BRX_PAL_SAMPLED_ASSET_IMAGE_SUBRESOURCE{destination_sampled_asset_image, mip_level_index};
        }

        upload_command_buffer->release(0U, NULL, static_cast<uint32_t>(uploaded_sampled_asset_image_subresources.size()), uploaded_sampled_asset_image_subresources.data(), 0U, NULL);

        graphics_command_buffer->acquire(0U, NULL, static_cast<uint32_t>(uploaded_sampled_asset_image_subresources.size()), uploaded_sampled_asset_image_subresources.data(), 0U, NULL);
    ...
    device->destroy_staging_upload_buffer(image_staging_upload_buffer);
    image_staging_upload_buffer = NULL;

    assert(NULL == this->m_image);
    this->m_image = destination_sampled_asset_image;
}

void brx_anari_pal_image::uninit(brx_pal_sampled_asset_image **const out_image)
{
    assert(NULL != this->m_image);
    (*out_image) = this->m_image;
    this->m_image = NULL;
}
