...
inline brx_pal_storage_asset_buffer *brx_anari_pal_device::create_asset_buffer(void const *const data_base, uint32_t const data_size)
{
    // TODO: batch
...
inline void brx_anari_pal_device::destroy_asset_buffer(brx_pal_storage_asset_buffer *const buffer)
{
#ifndef NDEBUG
    assert(!this->m_frame_throttling_index_lock);
    this->m_frame_throttling_index_lock = true;
...
inline void brx_anari_pal_surface::init(brx_anari_pal_device *device, BRX_ANARI_SURFACE const *surface)
{
    ...
    // Vertex Position Buffer
    {
        ...
        assert(NULL == this->m_vertex_position_buffer);
        this->m_vertex_position_buffer = device->create_asset_buffer(vertex_position_buffer_data.data(), sizeof(surface_vertex_position_buffer_element) * surface->m_vertex_count);
    }
    ...
    // Vertex Varying Buffer
    {
        ...
        assert(NULL == this->m_vertex_varying_buffer);
        this->m_vertex_varying_buffer = device->create_asset_buffer(vertex_varying_buffer_data.data(), sizeof(surface_vertex_varying_buffer_element) * surface->m_vertex_count);
    }
    ...
    if (NULL != surface->m_vertex_blendings)
    {
        ...
        assert(NULL == this->m_vertex_blending_buffer);
        this->m_vertex_blending_buffer = device->create_asset_buffer(vertex_blending_buffer_data.data(), sizeof(surface_vertex_blending_buffer_element) * surface->m_vertex_count);
    }
    else
    {
        assert(NULL == this->m_vertex_blending_buffer);
    ...
    // Index Buffer
    {
        ...
        if (raw_max_index <= static_cast<uint32_t>(UINT16_MAX))
        {
            ...
            assert(NULL == this->m_index_buffer);
            this->m_index_buffer = device->create_asset_buffer(uint16_indices.data(), sizeof(uint16_t) * surface->m_index_count);
        }
        else
        {
            assert(NULL == this->m_index_buffer);
            this->m_index_buffer = device->create_asset_buffer(surface->m_indices, sizeof(uint32_t) * surface->m_index_count);
        }
    }
    ...
    // Auxiliary Buffer
    {
        ...
        assert(NULL == this->m_auxiliary_buffer);
        this->m_auxiliary_buffer = device->create_asset_buffer(&surface_auxiliary_buffer_data, sizeof(surface_auxiliary_buffer));
    }
...
inline void brx_anari_pal_surface::uninit(brx_anari_pal_device *device)
{
    ...
    {
        assert(NULL != this->m_auxiliary_buffer);
        device->destroy_asset_buffer(this->m_auxiliary_buffer);
    ...
    {
        assert(NULL != this->m_index_buffer);
        device->destroy_asset_buffer(this->m_index_buffer);
    ...
    {
        assert(NULL != this->m_vertex_position_buffer);
        device->destroy_asset_buffer(this->m_vertex_position_buffer);
    ...
    {
        assert(NULL != this->m_vertex_varying_buffer);
        device->destroy_asset_buffer(this->m_vertex_varying_buffer);
    ...
    // Vertex Blending Buffer
    {
        if (NULL != this->m_vertex_blending_buffer)
        {
            device->destroy_asset_buffer(this->m_vertex_blending_buffer);
